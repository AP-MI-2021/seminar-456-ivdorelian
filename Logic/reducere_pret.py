from Domain.praitura import get_nume, get_pret, creeaza_prajitura, get_id, get_descriere, get_calorii, \
    get_an_introducere
...
def reducere_pret_pentru_nume(lst_prajituri, continut_nume, procentaj):
    ...
    for prajitura in lst_prajituri:
        if continut_nume in get_nume(prajitura):
            pret_nou = get_pret(prajitura) - (procentaj / 100) * get_pret(prajitura)
